...
[NOTE]
====
`spring-data-aerospike` is an Aerospike project and the dependency is not managed by Spring, it has its own release cycle
...
checked for the version of `spring-data-aerospike` that is compatible with the specific Spring Boot version, and for the documentation.
====
